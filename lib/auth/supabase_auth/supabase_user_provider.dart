...
class AdminAFSupabaseUser extends BaseAuthUser {
  AdminAFSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> adminAFSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = AdminAFSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
